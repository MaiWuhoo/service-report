 ...
 * stored in Firebase Storage (to stay under Firestore's 1 MiB per-doc
 * limit), so before generating a PDF we resolve every remote URL in the
 * report down to a data URL. Anything already a data URL (e.g. canvas
 * signatures) is left untouched.
 */
 ...
 *  NOTE: `report` here should already have Storage URLs resolved to data URLs
 *  (see resolveReportImages) — call this via getReportPDFBlobUrl /
 *  generateServiceReportPDF rather than directly if the report might contain
 ...
 *  Now async — it resolves any Firebase Storage URLs (photos/logos/stamp)
...
/** Now async for the same reason as getReportPDFBlobUrl above — await this
 *  at every call site. */
export async function generateServiceReportPDF(report, options = {}) {
  const resolved = await resolveReportImages(report);
  const doc = buildReportDoc(resolved, options);
